...
describe('maxDurationFor', () => {
  ...
  it('com o Lyria, o Premier cai para 3 min', () => {
    // O caso concreto que quebrou: o plano diz 480, o motor entrega 180.
    expect(PLANS.premier.features.maxDurationSeconds).toBe(480);
    expect(maxDurationFor('premier', 'lyria')).toBe(180);
  });

  it('com o ACE-Step, o Premier entrega os 8 min do plano', () => {
    expect(maxDurationFor('premier', 'acestep')).toBe(480);
  ...
  it('motor desconhecido cai no teto conservador, não no do plano', () => {
    // Uma configuração errada não pode liberar 8 min por acidente.
